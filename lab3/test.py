...
def runtest(test):
    try:
        subprocess.check_output('cp ./tests/' + test + '.rkt .', shell=True)
        subprocess.check_output('racket ' + test + '.rkt', shell=True, timeout=15)
        subprocess.call('rm ' + test + '.rkt', shell=True)
        return True
    except:
