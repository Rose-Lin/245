...
: public IMap<K,V> 
{
	...
	// TODO: write add method
      Link* node = root;
      if (!node){
      	root = new Link(key, val, 0);
	return;
      }
      for(;node && node->k != key; node=node->next);
      if (node){
	node -> v = val;
      }else{
	Link* tail = root;
	for(; tail->next; tail = tail->next);
	Link* newele = new Link(key, val, 0);
	tail -> next = newele;
      }
      return;
    } 
	...
	if (prev->k == key){
      ...
      for(; cur && cur->k != key;prev = prev->next, cur = cur->next){
	if(prev->k == key){
	  root = cur;
	  delete prev;
	  return 1;
	...
	// TODO: write size method
      unsigned size = 0;
      for (Link* node = root; node; node = node->next){
	size ++;
      }
      return size;
    }   	
